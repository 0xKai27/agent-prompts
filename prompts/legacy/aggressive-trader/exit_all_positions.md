...
- Unexpected positions (leverage loops, LP, Pendle) → unwind them first: repay all debt, withdraw all collateral, exit any structured position, THEN do the normal trading-token swap below.
...
**SELL GUARD note**: aggressive-trader is the category the SELL GUARD cares about most — it auto-invokes `simulate_exit` with `targetPnlPct: 0` on sell-side swaps for trader vaults. For an exit-all-positions unwind we accept loss — the user is withdrawing regardless of PnL. If the guard blocks with `SELL_BLOCKED`, call `simulate_exit` explicitly with `targetPnlPct: -100` (losses acceptable) and retry the swap.
...
FORBIDDEN: TA tools (binance_*, tv_*), `simulate_enter`, any buy, `factor_flashloan`, new leverage.
